 ...
 */
export function CompareView({ series, left, right, onLeftChange, onRightChange, plane, onPlaneChange, window: win, onWindowChange, overlays, overlayOpacity }: Props) {
  ...
  return (
    <>
      <div className="center-controls">
        ...
        <button className={`chip ${mode === "side" ? "active" : ""}`} onClick={() => setMode("side")}>
          Side by side
        </button>
        <button className={`chip ${mode === "swipe" ? "active" : ""}`} onClick={() => setMode("swipe")}>
          Swipe
        </button>
      ...
      {mode === "side" ? (
        <div className="compare-grid">
          <SliceCanvas volume={leftState.volume} plane={plane} index={leftIndex} window={win}
            ...
            overlays={overlays} overlayOpacity={overlayOpacity}
        ...
        <div className="swipe-box" ref={swipeBox} onPointerMove={onSplitDrag} onPointerDown={onSplitDrag}>
          <SliceCanvas volume={leftState.volume} plane={plane} index={leftIndex} window={win}
            progress={leftState.progress} error={leftState.error}
            overlays={overlays} overlayOpacity={overlayOpacity} className="swipe-layer"
            label={label(left, "Before", leftIndex, leftDepth)} />
          ...
          <div className="stage-label" style={{ left: `calc(${split * 100}% + 12px)` }}>
      ...
      <p className="hint">
        {sameStudy
          ? "Same round, two reconstructions: identical anatomy, different sharpening filter."
          : samePatient
            ? "Same person, different rounds. These slices are matched by position through the scan, not registered — the change overlay is the registered comparison."
